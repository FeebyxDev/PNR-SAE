				...
				if(constarr[0].size()>0) {
					...
					for(int i = 0; i < constarr[0].size(); i++) {
						...
						switch (arr.get(0)) {
							...
							case "CHECK":
								String dom = "";
								if(arr.get(2).equals("Domaine")) dom = "dom_";
								req += "CONSTRAINT ck_" + dom + arr.get(1) + " CHECK (" + arr.get(1) + " " + arr.get(3) + ")";
								break;
							case "FOREIGN KEY":
	...
	private HBox createConst() {
		...
		constType.setOnAction(e -> {
			switch (constType.getSelectionModel().getSelectedItem().toString()) {
				case "CHECK":
					MFXComboBox cbDom = new MFXComboBox();
					cbDom.setMinWidth(150);
					cbDom.floatModeProperty().set(FloatMode.BORDER);
					cbDom.floatingTextProperty().set("Type de l'attribut");
					cbDom.getItems().addAll("Domaine", "Classique");
					...
					hb.getChildren().add(hb.getChildren().size()-1, cbDom);
